...
func CreateMockedClient(doerInstance Doer) (metrixClient *metrix.Metrix, err error) {
	...
	metrixJSONRPC, err := metrix.NewClient(
		...
		metrix.SetDoer(doerInstance),
		metrix.SetDebug(isDebugEnvironmentVariableSet()),
	...
	metrixClient, err = metrix.New(metrixJSONRPC, "test")
	return
}
...
var (
	...
	GetTransactionByHashResponseWithTransactions = eth.GetBlockByHashResponse{
		...
		GasLimit:         utils.AddHexPrefix(metrix.DefaultBlockGasLimit),
		GasUsed:          "0x0",
	...
	GetBlockResponse = metrix.GetBlockResponse{
...
func MetrixTransactionReceipt(logs []metrix.Log) metrix.TransactionReceipt {
	return metrix.TransactionReceipt{
		BlockHash:         GetTransactionByHashBlockHexHash,
...
func SetupGetBlockByHashResponses(t *testing.T, mockedClientDoer Doer) {
	...
	getBlockHashResponse := metrix.GetBlockHashResponse(GetTransactionByHashBlockHexHash)
	err := mockedClientDoer.AddResponse(metrix.MethodGetBlockHash, getBlockHashResponse)
	if err != nil {
		t.Fatal(err)
	}

	getBlockHeaderResponse := metrix.GetBlockHeaderResponse{
		Hash:              GetTransactionByHashBlockHash,
		Confirmations:     1,
		Height:            3983,
		Version:           536870912,
	...
	err = mockedClientDoer.AddResponse(metrix.MethodGetBlockHeader, getBlockHeaderResponse)
	if err != nil {
		t.Fatal(err)
	}

	err = mockedClientDoer.AddResponse(metrix.MethodGetBlock, GetBlockResponse)
	if err != nil {
		t.Fatal(err)
	}

	getTransactionResponse := metrix.GetTransactionResponse{
		...
		Details: []*metrix.TransactionDetail{{Account: "",
			Category:  "send",
	...
	err = mockedClientDoer.AddResponse(metrix.MethodGetTransaction, getTransactionResponse)
	if err != nil {
		t.Fatal(err)
	}

	decodedRawTransactionResponse := metrix.DecodedRawTransactionResponse{
		ID:       "11e97fa5877c5df349934bafc02da6218038a427e8ed081f048626fa6eb523f5",
		...
		Version:  2,
		Locktime: 608,
		Vins: []*metrix.DecodedRawTransactionInV{{
		...
		Vouts: []*metrix.DecodedRawTransactionOutV{},
	}
	err = mockedClientDoer.AddResponse(metrix.MethodDecodeRawTransaction, decodedRawTransactionResponse)
	if err != nil {
		t.Fatal(err)
	}

	getTransactionReceiptResponse := metrix.GetTransactionReceiptResponse{}
	err = mockedClientDoer.AddResponse(metrix.MethodGetTransactionReceipt, &getTransactionReceiptResponse)
	if err != nil {
		t.Fatal(err)
	}

	// TODO: Get an actual response for this (only addresses are used in this test though)
	getRawTransactionResponse := metrix.GetRawTransactionResponse{
		Vouts: []metrix.RawTransactionVout{
			{
				Details: struct {
	...
	err = mockedClientDoer.AddResponse(metrix.MethodGetRawTransaction, &getRawTransactionResponse)
	if err != nil {
		t.Fatal(err)
	}
}
