...
func (p *ProxyETHGetBlockByHash) request(req *eth.GetBlockByHashRequest) (*eth.GetBlockByHashResponse, error) {
	...
	resp := &eth.GetBlockByHashResponse{
		...
		// ? Is it possible case for Metrix
	...
	if blockHeader.IsGenesisBlock() {
		...
		resp.Miner = utils.AddHexPrefix(metrix.ZeroAddress)
	} else {
	...
	resp.GasLimit = utils.AddHexPrefix(metrix.DefaultBlockGasLimit)
	resp.GasUsed = "0x0"

	if req.FullTransaction {
		for _, txHash := range block.Txs {
			tx, err := getTransactionByHash(p.Metrix, txHash)
			if err != nil {
			...
			if tx == nil {
				...
				if !p.GetFlagBool(metrix.FLAG_IGNORE_UNKNOWN_TX) {
		...
		for _, txHash := range block.Txs {
			...
			// 	has [64]byte just like Metrix tx hash has. In this case we do no
