...
func TestGetLogs(t *testing.T) {
	...
	//Add response
	err = clientDoerMock.AddResponseWithRequestID(2, metrix.MethodSearchLogs, searchLogsResponse)
	if err != nil {
		t.Fatal(err)
	}
	...
	proxyEth := ProxyETHGetLogs{metrixClient}
...
func TestGetLogsTranslateTopicWorksWithNil(t *testing.T) {
	...
	metrixRequest, err := proxyEth.ToRequest(&request)
	if err != nil {
		t.Fatal(err)
	}

	metrixRawRequest, err := json.Marshal(metrixRequest)
	if err != nil {
		t.Fatal(err)
	}
	...
	if expectedRawRequest != string(metrixRawRequest) {
		t.Errorf(
			"error\ninput: %s\nwant: %s\ngot: %s",
			metrixRawRequest,
			string(internal.MustMarshalIndent(expectedRawRequest, "", "  ")),
			string(internal.MustMarshalIndent(string(metrixRawRequest), "", "  ")),
		)
	}
}
