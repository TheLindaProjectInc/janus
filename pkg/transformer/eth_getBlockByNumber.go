...
func (p *ProxyETHGetBlockByNumber) request(req *eth.GetBlockByNumberRequest) (*eth.GetBlockByNumberResponse, error) {
	blockNum, err := getBlockNumberByRawParam(p.Metrix, req.BlockNumber, false)
	if err != nil {
	...
	var (
		...
		proxy = &ProxyETHGetBlockByHash{Metrix: p.Metrix}
	)
...
func proxyETHGetBlockByHash(p ETHProxy, q *metrix.Metrix, blockNum *big.Int) (*metrix.GetBlockHashResponse, error) {
	resp, err := q.GetBlockHash(blockNum)
	if err != nil {
		if err == metrix.ErrInvalidParameter {
