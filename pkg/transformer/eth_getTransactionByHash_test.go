...
//Insert some output in []*metrix.DecodedRawTransactionOutV() to force test cover more code
*/
func TestGetTransactionByHashRequest(t *testing.T) {
	...
	proxyEth := ProxyETHGetTransactionByHash{metrixClient}
...
/*
// TODO: Removing this unit test as the transformer computes the "Amount" value (how much MRX was transferred out) from the MethodDecodeRawTransaction response
...
func TestGetTransactionByHashRequest_PrecisionOverflow(t *testing.T) {
	...
	metrixClient, err := createMockedClient(mockedClientDoer)
	...
	err = mockedClientDoer.AddResponseWithRequestID(2, metrix.MethodGetTransaction, getTransactionResponse)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = mockedClientDoer.AddResponseWithRequestID(3, metrix.MethodDecodeRawTransaction, decodedRawTransactionResponse)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = mockedClientDoer.AddResponseWithRequestID(4, metrix.MethodGetBlock, getBlockResponse)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = mockedClientDoer.AddResponseWithRequestID(4, metrix.MethodGetRawTransaction, &getRawTransactionResponse)
	if err != nil {
		t.Fatal(err)
	}
