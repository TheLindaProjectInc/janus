...
func (p *ProxyETHGetFilterChanges) requestBlockFilter(filter *eth.Filter) (metrixresp eth.GetFilterChangesResponse, err error) {
	metrixresp = make(eth.GetFilterChangesResponse, 0)

	_lastBlockNumber, ok := filter.Data.Load("lastBlockNumber")
	if !ok {
		return metrixresp, errors.New("Could not get lastBlockNumber")
	}
	lastBlockNumber := _lastBlockNumber.(uint64)

	blockCountBigInt, err := p.GetBlockCount()
	if err != nil {
		return metrixresp, err
	}
...
func (p *ProxyETHGetFilterChanges) requestFilter(filter *eth.Filter) (metrixresp eth.GetFilterChangesResponse, err error) {
	metrixresp = make(eth.GetFilterChangesResponse, 0)
...
func (p *ProxyETHGetFilterChanges) doSearchLogs(req *metrix.SearchLogsRequest) (eth.GetFilterChangesResponse, error) {
	resp, err := p.SearchLogs(req)
	if err != nil {
		return nil, err
	}

	receiptToResult := func(receipt *metrix.TransactionReceipt) []interface{} {
...
func (p *ProxyETHGetFilterChanges) toSearchLogsReq(filter *eth.Filter, from, to *big.Int) (*metrix.SearchLogsRequest, error) {
	...
	metrixreq := &metrix.SearchLogsRequest{
		Addresses: addresses,
		FromBlock: from,
		ToBlock:   to,
	}
	...
	if ok {
		metrixreq.Topics = topics.([]interface{})
	}

	return metrixreq, nil
}
