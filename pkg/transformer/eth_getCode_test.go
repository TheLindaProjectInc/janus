...
func TestGetAccountInfoRequest(t *testing.T) {
	...
	proxyEth := ProxyETHGetCode{metrixClient}
...
func TestGetCodeInvalidAddressRequest(t *testing.T) {
	...
	//prepare responses
	getAccountInfoErrorResponse := metrix.GetErrorResponse(metrix.ErrInvalidAddress)
	if getAccountInfoErrorResponse == nil {
		panic("mocked error response is nil")
	}
	err = mockedClientDoer.AddError(metrix.MethodGetAccountInfo, getAccountInfoErrorResponse)
	if err != nil {
		t.Fatal(err)
	}
