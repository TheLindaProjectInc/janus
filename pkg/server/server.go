...
func New(
	...
	p := &Server{
		...
		echo:          echo.New(),
		address:       addr,
		metrixRPCClient: metrixRPCClient,
...
func (s *Server) Start() error {
	...
	e.Use(middleware.BodyDump(func(c echo.Context, req []byte, res []byte) {
		...
		if s.debug {
			reqBody, err := metrix.ReformatJSON(req)
			resBody, err := metrix.ReformatJSON(res)
			if err == nil {
	...
	level.Warn(s.logger).Log("listen", s.address, "metrix_rpc", s.metrixRPCClient.URL, "msg", "proxy started", "https", https)
