...
func (s *subscriptionInformation) run() {
	...
	nextBlock := 0
	metrixTopics, err := eth.TranslateTopics(s.params.Params.Topics)
	if err != nil {
		s.metrix.GetDebugLogger().Log("msg", "Error translating logs topics", "error", err)
		return
	}
	req := &metrix.WaitForLogsRequest{
		FromBlock: nextBlock,
		ToBlock:   "latest",
		Filter: metrix.WaitForLogsFilter{
			Topics: &metrixTopics,
		},
	}

	// this throttles METRIX api calls if waitforlogs is returning very quickly a lot
	limitToXApiCalls := 5
	inYSeconds := 10 * time.Second
	// if a METRIX API call returns quicker than this, we will wait until this time is reached
	// this prevents spamming the METRIX node too much
	...
	for {
		...
		resp, err := s.metrix.WaitForLogsWithContext(s.ctx, req)
		timeAfterCall := time.Now()
		if err == nil {
			for _, metrixLog := range resp.Entries {
				ethLogs := conversion.ExtractETHLogsFromTransactionReceipt(&metrixLog)
				for _, ethLog := range ethLogs {
					...
					if _, ok := sentHashes[hash]; !ok {
						...
						s.metrix.GetDebugLogger().Log("subscriptionId", s.id, "msg", "notifying of logs")
						s.Send(subscription)
					}
				}
			}
			...
			// error occurred
			s.metrix.GetDebugLogger().Log("subscriptionId", s.id, "err", err)
			failures = failures + 1
		}
		...
		case <-done:
			...
			s.metrix.GetDebugLogger().Log("subscriptionId", s.id, "msg", "context closed, dropping subscription")
			return
		default:
		}
		...
		if backoffTime > 0 {
			s.metrix.GetDebugLogger().Log("subscriptionId", s.id, "msg", fmt.Sprintf("backing off for %d miliseconds", backoffTime/time.Millisecond))
		}
