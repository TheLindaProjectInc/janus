...
func TestAgentAddSubscriptionLogs(t *testing.T) {
	...
	doer.AddResponse(metrix.MethodWaitForLogs, metrix.WaitForLogsResponse{
		Entries: []metrix.TransactionReceipt{
			internal.MetrixTransactionReceipt([]metrix.Log{
				{
					Address: internal.MetrixTransactionReceipt(nil).ContractAddress,
		...
		Count:     1,
		NextBlock: internal.MetrixTransactionReceipt(nil).BlockNumber + 1,
	})
...
func TestAgentAddSubscriptionNewHeads(t *testing.T) {
	...
	for i := int64(1); i < 10; i++ {
		doer.AddResponse(metrix.MethodGetBlockChainInfo, metrix.GetBlockChainInfoResponse{
			Blocks:        i,
			Bestblockhash: "0x1",
		})
	}
