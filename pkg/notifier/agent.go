...
func NewAgent(ctx context.Context, metrix *metrix.Metrix, transformer Transformer) *Agent {
	return newAgentWithConfiguration(ctx, metrix, transformer, make(map[string]interface{}))
}

func newAgentWithConfiguration(ctx context.Context, metrix *metrix.Metrix, transformer Transformer, configuration map[string]interface{}) *Agent {
	if ctx == nil {
		panic("ctx cannot be nil")
	}
	if metrix == nil {
		panic("metrix cannot be nil")
	}
	agent := &Agent{
		metrix:          metrix,
...
func (a *Agent) run() {
	...
	defer func() {
		...
		a.metrix.GetDebugLogger().Log("msg", "Agent exited subscription processing thread")
	...
	a.metrix.GetDebugLogger().Log("msg", "Agent started subscription processing thread")
	...
	for {
		...
		a.mutex.RUnlock()
		if transformer == nil {
			a.metrix.GetErrorLogger().Log("msg", "Agent does not have access to eth transformer, cannot process 'newHeads' subscriptions")
		} else {
			blockchainInfo, err := a.metrix.GetBlockChainInfo()
			if err != nil {
				a.metrix.GetErrorLogger().Log("msg", "Failure getting blockchaininfo", "err", err)
			} else {
				latestBlock := blockchainInfo.Blocks
				if lastBlock == 0 {
					// prevent sending the current head to the first client connected
					lastBlock = latestBlock
					a.metrix.GetDebugLogger().Log("msg", "Got getblockchaininfo response for same block", "block", lastBlock)
				} else if latestBlock > lastBlock {
					a.metrix.GetDebugLogger().Log("msg", "New head detected", "block", latestBlock)
					...
					if err != nil {
						a.metrix.GetErrorLogger().Log("msg", "Failed to eth_getBlockByHash", "hash", blockchainInfo.Bestblockhash, "err", err)
					} else {
						getBlockByHashResponse, ok := result.(*eth.GetBlockByHashResponse)
						if !ok {
							a.metrix.GetErrorLogger().Log("msg", "Failed to eth_getBlockByHash, unexpected response type", "hash", blockchainInfo.Bestblockhash)
						} else {
							lastBlock = latestBlock
							// notify newHead
					...
					a.metrix.GetDebugLogger().Log("msg", "Detected same head", "block", latestBlock)
				}
			}
		}
