...
func (c *Client) RequestWithContext(ctx context.Context, method string, params interface{}, result interface{}) error {
	...
	for i := 0; i < max; i++ {
		...
		if err != nil {
			if strings.Contains(err.Error(), ErrMetrixWorkQueueDepth.Error()) && i != max-1 {
				requestString := marshalToString(req)
				backoffTime := computeBackoff(i, true)
				c.GetLogger().Log("msg", fmt.Sprintf("METRIX process busy, backing off for %f seconds", backoffTime.Seconds()), "request", requestString)
				time.Sleep(backoffTime)
				c.GetLogger().Log("msg", "Retrying METRIX command")
			} else {
				if i != 0 {
					c.GetLogger().Log("msg", fmt.Sprintf("Giving up on METRIX RPC call after %d tries since its busy", i+1))
				}
				return err
			}
		} else {
...
func (c *Client) Do(ctx context.Context, req *JSONRPCRequest) (*SuccessJSONRPCResult, error) {
	...
	if c.IsDebugEnabled() {
		...
		if err == nil {
			fmt.Printf("<= metrix RPC response\n%s\n", formattedBodyStr)
		}
	}
	...
	if err != nil {
		...
		if string(respBody) == ErrMetrixWorkQueueDepth.Error() {
			// METRIX http server queue depth reached, need to retry
			return nil, ErrMetrixWorkQueueDepth
		}
...
func (c *Client) do(ctx context.Context, body io.Reader) ([]byte, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "ioutil error in metrix client package")
	}
	return reader, nil
}
...
func SetLogger(l log.Logger) func(*Client) error {
	return func(c *Client) error {
		c.logger = log.WithPrefix(l, "component", "metrix.Client")
		return nil
	}
}
...
func checkRPCURL(u string) error {
	...
	metrixRPC, err := url.Parse(u)
	if err != nil {
		return errors.Errorf("METRIX_RPC URL: %s", u)
	}

	if metrixRPC.User == nil {
		return errors.Errorf("METRIX_RPC URL (must specify user & password): %s", u)
	}
