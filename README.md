...
```
$ go get github.com/TheLindaProjectInc/janus/...
$ cd $GOPATH/src/github.com/TheLindaProjectInc/janus/playground
$ make
$ make docker-dev
$ make quick-start
```
...
-   Another one named `metrix` running on port 33831
    

`make quick-start` will also fund the tests accounts with MRX in order for you to start testing and developing locally. Additionally, if you need or want to make changes and or additions to Janus, but don't want to go through the hassle of rebuilding the container, you can run the following command at the project root level:
```
$ make run-janus
# For https
...
- Sending coins with the creation of a contract will cause a loss of coins
  - This is a Metrix intentional deisgn decision and will not change
- On a transfer of Metrix to a Metrix address, there is no receipt generated for such a transfer
- When converting from WEI -> MRX, precision is lost due to MRX's smallest demonination being 1 satoshi.
  - 1 satoshi = 0.00000001 MRX = 10000000000 wei
- MRX's minimum gas price is 5000 satoshi
  - When specifying a gas price in wei lower than that, the minimum gas price will be used (5000 satoshi)
